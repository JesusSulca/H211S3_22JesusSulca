...
('P0003','P','PIZZA PEPPERONI ROMANZZA',70,'A',150,'A','De pepperoni, acompañado con tomates frescos y champiñones .'),
('P0004','P','4 ESTACIONES',32,'F',150,'A','Pizza: súper suprema, canadiense, pepperoni y jamón.'),
('P0005','P','SUPER SUPREMA',45,'F',150,'A','Hecho con pepperoni, carne de res y cerdo.'),
('P0006','P','CANADIENSE',48,'F',150,'A','Elaborada con pepperoni, jamón, carne de res y cerdo, salchicha italiana y bacon bits..'),
('P0007','P','SUPREMA',51,'F',150,'A','Elaborada con pepperoni, carne de res y cerdo, champiñones, chile verde y cebolla.'),
...
select v.IDVEN,
concat(c.NOMCLI , ' , ', c.APECLI) as Cliente,
concat(ve.NOMVEND , ' , ' , ve.APEVEND) as Vendedor ,
p.NOMPRO,  p.PREPRO, 
vd.CANTPRO,
SUBTOTAL = (p.PREPRO * vd.CANTPRO)
from VENTA_DETALLE vd 
inner join venta v on v.IDVEN = vd.IDVEN
inner join PRODUCTO p on p.CODPRO = vd.CODPRO
inner join cliente c on c.IDCLI = v.IDCLI
inner join VENDEDOR ve on ve.IDVEND = v.IDVEND ;
Go

Create view pActivo as
select * from producto where ESTPRO = 'A';
Go

Create view pInactivo as
select * from producto where ESTPRO = 'I';
Go
